{
  ...
  const user2: {
    company: "DevSafi"; // Literal type: 'company' can only be "DevSafi"
    readonly address: string; // Readonly type: 'address' cannot be changed after initialization
    firstName: string;
    lastName: string;
    age: number;
    email?: string; // Optional property: 'email' is optional
  } = {
